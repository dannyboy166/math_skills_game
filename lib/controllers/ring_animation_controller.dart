...
class RingAnimationController {
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      final position = isInner
          ? SquarePositionUtils.calculateInnerSquarePosition(i, size, tileSize,
              cornerSizeMultiplier: _targetSizes[i]!, margin: margin)
  ...
  void prepareAnimation({
    ...
    required double tileSize,
    required bool isInner,
    required double margin, // 👈 ADD THIS
  }) {
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (previousPosition != null) {
        ...
        final wasPreviousCorner =
            ringModel.cornerIndices.contains(previousPosition);
        ...
        _currentOpacities[i] =
            wasPreviousCorner ? _cornerOpacity : _regularOpacity;
        ...
        final oldPosition = isInner
            ? SquarePositionUtils.calculateInnerSquarePosition(
                ...
                cornerSizeMultiplier: _currentSizes[i]!, margin: margin)
  ...
  AnimatedTileInfo getAnimatedTileInfo(
      int index, bool isCorner, bool isLocked) {
    // Offset position
    ...
    // Size
    double startSize =
        _currentSizes[index] ?? (isCorner ? _cornerSize : _regularSize);
    double endSize =
        _targetSizes[index] ?? (isCorner ? _cornerSize : _regularSize);

    // Opacity
    double startOpacity = _currentOpacities[index] ??
        (isCorner ? _cornerOpacity : _regularOpacity);
    double endOpacity = _targetOpacities[index] ??
        (isCorner ? _cornerOpacity : _regularOpacity);
